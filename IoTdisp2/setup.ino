...
void setupdisp()
{
    ...
    if(mandet == 0){
        drawbtn(10, BLACK, LIGHTGREY, "検知オフ");
    }
    else{
        drawbtn(10, BLACK, CYAN, "検知オン");
    }
    if(buzzer == 0){
...
// ボタン描画
// num : 時計ボタン（0～3）検知（10、11）センサー（20～24）音量（30～32）
// col1 : 背景色
// col2 : 文字色
// str : 表示文字
void drawbtn(int num, int col1, int col2, char *str)
{
    ...
    if(num < 10){
        ...
        if(earth[num-20] != 0){  // 土壌湿度センサー
            display.fillRect(sensbtn[num-20][0], sensbtn[num-20][1]+sensbtn[num-20][3]-50, sensbtn[num-20][2], 50, CYAN);
        }
        int w = display.textWidth(str);
            ...
            if(nowpos == 0){  // 時計設定
                if(*md == 0){
                    *md = 1;   // 設定モード
                    ...
                    if(mandet == 0){
                        drawbtn(10, BLACK, DARKGREY, "検知オフ");
                    }
                    else{
                        drawbtn(10, BLACK, DARKCYAN, "検知オン");
                    }
                    if(buzzer == 0){
